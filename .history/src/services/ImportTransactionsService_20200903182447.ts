...
class ImportTransactionsService {
  public async execute(filePath: string): Promise<Transaction[]> {
    ...
    const transactions: CSVTransaction[] = []; // esses dois vetores foram criados para armazenar o dado e adicionar no banco tudo de uma vez, por mais que da outra maneira seja simples, que é ficar adicionando informação por informação no banco, ele vai ficar abrindo muitas conexões e isso não é legal.
    const categories: string[] = [];

    // abaixo  através do on, ele irá percorrer linha por linha que retornar e estiver disponivel e iremos desustruturar cada informação de cada linha que tiver
    parseCSV.on('data', async line => {
      const [title, type, value, category] = line.map((cell: string) =>
        cell.trim(),
      ); // agora iremos percorrer todas as linhas checando cada celula desse documento.csv
      // remove todo o espaço em banco que tiver em cada celula
